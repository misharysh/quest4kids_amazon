...
export class UpdateTaskHandler implements ICommandHandler<UpdateTaskCommand, Task> {
    ...
    async execute(command: UpdateTaskCommand): Promise<Task> {
        ...
        const userForComment = await this.usersRepository.findOneBy({ id: task.userId });
        ...
        if (taskData.status) {
            ...
            await this.statusLogger.createStatusLog(user, task, taskData.status, task.status);
        
            if (user?.role === Role.CHILD && user.parentId) {
                const parent = await this.usersRepository.findOneBy({ id: user.parentId });
            ...
            if (taskData.status === TaskStatus.DONE && user) {
                ...
                await this.commandBus.execute( // ⬅️ изменено: теперь SetReward вызывается как отдельная команда
    ...
    private isValidStatusTransition(current: TaskStatus, next: TaskStatus): boolean {
        const statusOrder = [
            TaskStatus.OPEN,
