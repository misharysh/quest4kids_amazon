...
export class AwsService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.s3 = new AWS.S3({
      ...
      secretAccessKey:
        configService.get<AmazonConfig>('amazon')?.secretAccessKey,
    });
  }
  ...
  public async s3_upload(file, name, mimeType) {
    ...
    if (bucket) {
      ...
      try {
        const s3Repsonse = await this.s3.upload(params).promise();

        return s3Repsonse;
      } catch (e) {
        throw new InternalServerErrorException(
          'Error during uploading of avatar to S3',
        );
      }
    } else {
  ...
  public async s3_get(key: string): Promise<string> {
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Error during getting of avatar from S3',
      );
    }
  }
  ...
  public async s3_delete(key: string): Promise<void> {
    try {
      ...
      throw new InternalServerErrorException(
        'Error during deleting of avatar from S3',
      );
    }
  }
}
